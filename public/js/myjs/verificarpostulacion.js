        ...
        success: function(data) {
        	...
        	if(data.postulacion.estado == 2){
		    	...
		    	$htmlPostulante = data.postulante.nombre+" "+data.postulante.apepaterno+" "+data.postulante.apematerno+ 
    				"<br><small>Postulación Verificada: <strong>"+pad(data.postulacion.nroPostulante, 6)+"</strong></small></h1>"+
                ...
                if(data.iscoordinador == 1)
                	$htmlPostulante += "<p style='margin: 0px 0px 20px 20px;'>"+
                      "<button onclick='editar("+data.postulacion.nroPostulante+")' class='btn btn-primary btn-xs'> <span class='fa fa-pencil'></span> Editar</button>"+
                      "</p>";
...
if($postulante.extranjero == 0)
	...
	//AGREGADO 07/09/2018
	$('#medioseentero').val($postulacion.medioseentero);
	$('#dondesepreparo').val($postulacion.dondesepreparo);
}
